...
<aside class="bg-white/95 border-r border-gray-200 h-screen sticky top-0 flex flex-col backdrop-blur">
  ...
  [
    ...
    ['/empresas', 'Empresas', 'fa-solid fa-building', ['/empresas']], // NUEVO
  ],
  ['/division', '/divisiones', '/cargos', '/empresas']
);
      ...
      if (strtolower((string)($user['role'] ?? '')) === 'admin') {
        nav_item('/usuarios/index.php', 'Usuarios', 'fa-solid fa-user-gear', ['/usuarios', '/usuarios/index.php']);
      }
    ?>
  </nav>
  ...
  <div class="p-4 border-t text-sm text-gray-700">
    ...
    <div class="text-gray-500">
      Rol: <?= htmlspecialchars($user['role_label'] ?? ($user['role'] ?? '-'), ENT_QUOTES, 'UTF-8'); ?>
    </div>
  </div>
</aside>
